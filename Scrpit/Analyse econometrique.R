...
consommation_saison <- consommation_jour %>%
  ...
  arrange(saison, Date)
options(scipen = 999)
reg_conso_futur <- lm(Y_consommation_future~X1_consommation_passe+X2_weekend+saison,data = consommation_saison)
summary(reg_conso_futur)

conso_2semaine_prediction <- predict(reg_conso_futur,newdata = data.frame(X1_consommation_passe=135.0714,X2_weekend=1,saison="Printemps"))
